...
bool BlockIter<TValue>::ParseValue_leco(int index) {
  ...
  uint32_t offset_val_off = reinterpret_cast<const uint32_t*>(data_ +  data_size + block_offset)[0];
  uint64_t offset_val;
  randomdecodeArray8_integer(data_ + data_size + data_offset + offset_val_off, index % block_size_, &offset_val, index_block_num);

  uint32_t size_val_off = reinterpret_cast<const uint32_t*>(data_ + off_size + block_offset)[0];
  uint64_t size_val;
...
IndexBlockIter* Block::NewIndexIterator(
    ...
    bool using_leco_encode, int block_size, bool padding_enable, int key_num_per_block) {
  ...
  if (num_restarts_ == 0) {
    ...
    ret_iter->Initialize(raw_ucmp, data_, restart_offset_, num_restarts_,
                         ...
                         using_leco_encode, block_size, padding_enable, key_num_per_block);
  }
