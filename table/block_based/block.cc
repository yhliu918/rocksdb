...
void IndexBlockIter::SeekImpl(const Slice& target) {
  ...
  if (prefix_index_) {
    ...
    ok = BinarySeek_leco(seek_key, &index);
    // std::cout<< "key "<< seek_key.ToString() << " index " << index <<
    // std::endl;
    skip_linear_scan = true;
    if (!ok) {
      status_ = Status::NotFound();
...
bool BlockIter<TValue>::ParseValue_leco(int index) {
  ...
  offset_ = offset_val;
  size_ = size_val;
  // std::cout<<"index "<<index<<" offset "<<offset_<<" size
  // "<<size_<<std::endl; uint64_t offset, size; GetFixed64(&value_,&offset);
...
bool pre_Bsearch(int left, int right, int* index, const char* key,
                 ...
                 const char* firstkey_each_block,
                 int search_len) {  // check which block the key belongs to
  while (left != right) {
    // The `mid` is computed by rounding up so it lands in (`left`, `right`].
    int64_t mid = left + (right - left + 1) / 2;

    // uint8_t first_key_length = reinterpret_cast<const
    // uint8_t*>(firstkey_each_block+ mid * string_len)[0]; const size_t min_len
    // = (first_key_length < search_len) ? first_key_length : search_len; int
    // cmp = memcmp(firstkey_each_block + mid * string_len + 1, key, min_len);
    // if (cmp == 0) {
    ...
    // }
    int cmp =
        memcmp(firstkey_each_block + mid * string_len + 1, key, search_len);

    // std::string string_comp(firstkey_each_block + mid * string_len + 1,
    // search_len); std::cout<< search_len<<" "<<string_comp<<std::endl; need
    // another length info when storing each first key.
    if (cmp < 0) {
      // Key at "mid" is smaller than "target". Therefore all
      // blocks before "mid" are uninteresting.
      left = mid;
      ...
      uint8_t first_key_length = reinterpret_cast<const uint8_t*>(
          firstkey_each_block + mid * string_len)[0];
      if (first_key_length < search_len)
        left = mid;
      else if (first_key_length > search_len)
        right = mid - 1;
...
// The structure of Index block using Leco is like:
// We need two param (padding_enable, include_first_key) to control the behavior
// of decoding the block DEFAULT SETTING FOR KEYS: padding_enable = true(because
...
template <class TValue>
bool BlockIter<TValue>::BinarySeek_leco(Slice& target, uint32_t* index) {
  uint32_t N;
  N = reinterpret_cast<const uint32_t*>(data_)[0];
  int block_number = N / block_size_;
  while (block_size_ * block_number < N) {
  ...
  // compressed block[block_ind] = data_[data_offset + N + block_num * 4 +
  // start_byte[block_ind] ]

  // std::string target_str(target.data(), target.size());
  bool skip_linear = false;
  int index_search = 0;
  if (padding_enable_) {
    pre_Bsearch(-1, total_firstkey_num - 1, &index_search, target.data(),
                &skip_linear, max_padding_length + sizeof(uint8_t),
                data_ + sizeof(uint32_t) * 4 + sizeof(uint8_t), target.size());
  }
  if (!skip_linear) {
    ...
    uint8_t encoding_type = reinterpret_cast<const uint8_t*>(
    ...
    switch (encoding_type) {
      case 0:{
        ...
        while (left != right) {
          // The `mid` is computed by rounding up so it lands in (`left`,
          // `right`].
...
BlockBasedTableOptions::DataBlockIndexType Block::IndexType() const {
  if (using_leco_encode_) {
    return BlockBasedTableOptions::kDataBlockLeco;
  }
  assert(size_ >= 2 * sizeof(uint32_t));
  if (size_ > kMaxBlockSizeSupportedByHashIndex) {
...
Block::Block(BlockContents&& contents, size_t read_amp_bytes_per_bit,
             Statistics* statistics, bool using_leco_encode)
...
IndexBlockIter* Block::NewIndexIterator(
    ...
    bool using_leco_encode, int block_size, bool padding_enable,
