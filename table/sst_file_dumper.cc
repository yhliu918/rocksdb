...
Status SstFileDumper::DumpTable(const std::string& out_filename) {
  ...
  s = env->NewWritableFile("/home/lyh/rocksdb/dump_data/mail_server_host_reverse_min_10_max_26_key.txt", &out_file_key, soptions_);
  if (!s.ok()) {
    // close the file before return error, ignore the close error if there's any
    out_file_key->Close().PermitUncheckedError();
    return s;
  }
  s = env->NewWritableFile("/home/lyh/rocksdb/dump_data/mail_server_host_reverse_min_10_max_26_offset.txt", &out_file_offset, soptions_);
  if (!s.ok()) {
    // close the file before return error, ignore the close error if there's any
    out_file_offset->Close().PermitUncheckedError();
    return s;
  }
  s = env->NewWritableFile("/home/lyh/rocksdb/dump_data/mail_server_host_reverse_min_10_max_26_size.txt", &out_file_size, soptions_);
  if (!s.ok()) {
    // close the file before return error, ignore the close error if there's any
