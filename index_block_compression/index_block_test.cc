...
//       reader.NewDataIterator(options.comparator,
...
//     ROCKSDB_NAMESPACE::CreateDBStatistics(); BlockReadAmpBitmap
//     read_amp_bitmap(block_size, kBytesPerBit, stats.get());
...
//     auto it =
//         std::unique(random_entry_offsets.begin(),
//         random_entry_offsets.end());
...
//       auto &current_entry = random_entries[rnd.Next() %
//       random_entries.size()];
...
//   std::shared_ptr<Statistics> stats =
//   ROCKSDB_NAMESPACE::CreateDBStatistics(); ASSERT_EQ(BlockReadAmpBitmap(100,
//   1, stats.get()).GetBytesPerBit(), 1u); ASSERT_EQ(BlockReadAmpBitmap(100, 2,
//   stats.get()).GetBytesPerBit(), 2u); ASSERT_EQ(BlockReadAmpBitmap(100, 4,
//   stats.get()).GetBytesPerBit(), 4u); ASSERT_EQ(BlockReadAmpBitmap(100, 8,
//   stats.get()).GetBytesPerBit(), 8u); ASSERT_EQ(BlockReadAmpBitmap(100, 16,
//   stats.get()).GetBytesPerBit(), 16u); ASSERT_EQ(BlockReadAmpBitmap(100, 32,
//   stats.get()).GetBytesPerBit(), 32u);
...
//       options, "/home/zxy/rocksdb/index_block_compression/new_db_dir", &db);

//   if (!status.ok()) {
//     std::cout << "creating new DB\n";
//     options.create_if_missing = true;
//     status = rocksdb::DB::Open(
//         options, "/home/zxy/rocksdb/index_block_compression/new_db_dir",
//         &db);
//     assert(status.ok());
